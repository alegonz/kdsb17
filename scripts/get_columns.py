...
for p, patient_folder in enumerate(os.listdir(BASE_PATH), start=1):
    ...
    if p % 100 == 0:
        ...
        pd.DataFrame(columns=columns).to_csv('/sandbox/data_science_bowl_2007/analysis/header.csv.temp')

print('There are %d dcm files and %d different columns.' % (n,len(columns)))
pd.DataFrame(columns=columns).to_csv('/sandbox/data_science_bowl_2007/analysis/header.csv', index=False)
